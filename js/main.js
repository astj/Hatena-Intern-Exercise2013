...
function parseLTSVLog(GivenLTSVString) {
    ...
    for(var i = 0; i < GivenLTSVArray.length; i++){
        ...
        for(var j = 0; j < OneLineArray.length; j++ ) {
            var OneLineSplitted = OneLineArray[j].match(/(.+?):(.*)/);
            if(OneLineSplitted) {
                if(OneLineSplitted[1]=='path'){ OneLineObj['path'] = OneLineSplitted[2]; }
                if(OneLineSplitted[1]=='reqtime_microsec'){ OneLineObj['reqtime_microsec'] = +OneLineSplitted[2]; }
            }
        }
